---
title: "COVID HCW Data - Latent Profile Analysis"
...
### Read HCW longitudinal data
```{r}
rm(list=ls())

hcwData <- read.csv("HCW_timepoints_merged.csv", header = TRUE, na.strings=c("","NA"))
nrow(hcwData)
```
...
hcwData$ID[duplicated(hcwData$ID)]
```
...
hcwData<-hcwData %>% filter(redcap_event_name == "m0_arm_1")
```
### Mode function
```{r}
...
hcwData$MIOSshame_score[match(MIOSshame_temp$ID, hcwData$ID)] <- MIOSshame_temp$MIOSshame_score
#test the results
# test_MIOSShame<- hcwData %>% select(colnmsShame, MIOSshame_perc_missing, MIOSshame_rowMode, MIOSshame_score)
...
hcwData$MIOStrust_rowMode<- apply(hcwData[,colnmsTrust], 1, FUN = function(x) ModeFunc(na.omit(x)))

#impute missing values rowwise with rowmode for participants having less than 20% missing, then calculate scale total
hcwData$MIOStrust_score<-NA
colnmsTrust_plus<-c("ID", colnmsTrust , "MIOStrust_perc_missing", "MIOStrust_rowMode")
MIOStrust_temp<-hcwData[,colnmsTrust_plus]
...
hcwData$MIOStrust_score[match(MIOStrust_temp$ID, hcwData$ID)] <- MIOStrust_temp$MIOStrust_score

#test the results
# test_MIOStrust<- hcwData %>% select(colnmsTrust, MIOStrust_perc_missing, MIOStrust_rowMode, MIOStrust_score)
#### MIOS Total Score
...
##mios_total_score
#Calculate percent missing by dividing the number of NA cells by total cells, multiply by 100 and lastly round.
hcwData$mios_perc_missing <- round((rowSums(is.na(hcwData[,colnms_mios_14item]))/14)*100, 2)

#calculate the mode of each row for the mios items (14 items)
hcwData$mios_rowMode<- apply(hcwData[,colnms_mios_14item], 1, FUN = function(x) ModeFunc(na.omit(x)))

#impute missing values rowwise with rowmode for participants having less than 20% missing, then calculate scale total

hcwData$mios_score<-NA
colnms_mios_14item_plus<-c("ID", "record_id", colnms_mios_14item , "mios_perc_missing", "mios_rowMode")
mios_temp<-hcwData[,colnms_mios_14item_plus]
...
hcwData$mios_score[match(mios_temp$ID, hcwData$ID)] <- mios_temp$mios_score

#test the results
# test_MIOS<- hcwData %>% select(colnms_mios_14item, mios_rowMode, MIOStrust_rowMode, mios_score)
...
mios_selection<-hcwData %>%select(MIOSshame_score, MIOStrust_score)

#removing missing values
subscales <- mios_selection[complete.cases(mios_selection), ]

mios_profile1<-subscales %>% estimate_profiles(1:5 , variances = "equal" ,covariances = "zero")
mios_profile1
...
# get the data set including the estimated profiles(classes)
data_with_profiles<-mios_profile1$model_1_class_5$dff

#merge subscales data with classes
class_data<-data_with_profiles %>% select(Class)
final_result_sub<- cbind(subscales, class_data)
...
mios_temp1<-mios_temp %>%select(all_of(colnms_mios_14item))

#removing missing values
# df <- mios_temp[complete.cases(mios_temp), ]

#perform latent profile analysis on 14 mios items   varying
mios_profile2<-mios_temp1 %>% estimate_profiles(1:5 , variances = "equal" ,covariances = "zero")
mios_profile2
...
# get the data set including the estimated profiles(classes)
data_with_profiles<-mios_profile2$model_1_class_5$dff
#merge miostemp data with classes
class_data<-data_with_profiles %>% select(Class)
final_result<- cbind(mios_temp, class_data)
